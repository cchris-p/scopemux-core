...
TSTree *ts_parser_parse_string(TSParser *parser, const TSTree *old_tree, const char *string,
                               uint32_t length);
...
TSRange *ts_tree_get_changed_ranges(const TSTree *old_tree, const TSTree *new_tree,
                                    uint32_t *length);
...
TSNode ts_node_named_descendant_for_point_range(TSNode node, TSPoint start_point,
                                                TSPoint end_point);
...
void ts_query_predicates_for_pattern(const TSQuery *query, uint32_t pattern_index,
                                     uint32_t *length);
...
TSSymbol ts_language_symbol_for_name(const TSLanguage *language, const char *name, uint32_t length,
                                     bool is_named);
...
TSFieldId ts_language_field_id_for_name(const TSLanguage *language, const char *name,
                                        uint32_t length);
TSSymbol ts_language_symbol_for_field_id(const TSLanguage *language, TSFieldId field_id,
