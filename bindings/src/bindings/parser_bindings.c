 ...
 * @brief Parse a file
 */
static PyObject* ParserContext_parse_file(ParserContextObject* self, PyObject* args, PyObject* kwds) {
    const char* filename;
 ...
 */
static PyObject* ParserContext_parse_string(ParserContextObject* self, PyObject* args, PyObject* kwds) {
    const char* content;
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds, "s#|si", kwlist, &content, &content_length, &filename, &language)) {
        return NULL;
    }
    
    bool success = parser_parse_string(self->context, content, content_length, filename, (LanguageType)language);
    if (!success) {
        PyErr_SetString(PyExc_RuntimeError, parser_get_last_error(self->context));
        return NULL;
    }
 ...
 */
static PyObject* ParserContext_get_nodes_by_type(ParserContextObject* self, PyObject* args, PyObject* kwds) {
    int node_type;
...
static PyMethodDef ParserContext_methods[] = {
    ...
    {"get_nodes_by_type", (PyCFunction)ParserContext_get_nodes_by_type, METH_VARARGS | METH_KEYWORDS, 
...
static PyObject* IRNode_get_range(IRNodeObject* self, void* closure) {
    ...
    PyDict_SetItemString(start_dict, "column", PyLong_FromUnsignedLong(self->node->range.start.column));
    PyDict_SetItemString(start_dict, "offset", PyLong_FromUnsignedLong(self->node->range.start.offset));
