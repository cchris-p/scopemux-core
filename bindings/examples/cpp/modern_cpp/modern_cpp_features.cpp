...
void demonstrate_smart_pointers() {
    ...
    std::cout << "\nstd::weak_ptr:" << std::endl;
    {
        ...
        shared.reset();
        std::cout << "After shared.reset(), weak ptr is " << (weak.expired() ? "expired" : "valid") << std::endl;
...
void demonstrate_structured_bindings() {
    ...
    std::cout << "Tuple decomposition: id=" << id << ", value=" << value << ", name=" << name << std::endl;
...
bool is_any_equal(T first, Args... args) {
    return (... || (first == args));  // Unary right fold with || operator
}

template<typename T, typename... Args>
bool are_all_equal(T first, Args... args) {
    return (... && (first == args));  // Unary right fold with && operator
}
...
void demonstrate_if_constexpr_and_fold() {
    ...
    std::cout << "Is any equal to 10 in (5, 10, 15)? " 
              << std::boolalpha << is_any_equal(10, 5, 10, 15) << std::endl;
    std::cout << "Is any equal to 20 in (5, 10, 15)? " 
              << is_any_equal(20, 5, 10, 15) << std::endl;
    
    std::cout << "Are all equal in (5, 5, 5)? " 
              << are_all_equal(5, 5, 5) << std::endl;
    std::cout << "Are all equal in (5, 5, 10)? " 
              << are_all_equal(5, 5, 10) << std::endl;
}
