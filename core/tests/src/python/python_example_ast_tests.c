...
static void test_python_example(const char *category, const char *filename) {
  ...
  if (env_source_file && env_json_file) {
    ...
    paths = construct_test_paths("python", category, filename);
    if (!paths.base_filename) {
...
static bool extract_test_info(const char *test_file_path, char **category, char **filename) {
  ...
  const char *pattern = "core/tests/examples/python/";
...
/**
 * Test that processes all Python example files or a specific file based on environment variable
 */
Test(python_examples, all_examples) {
  ...
  if (test_file_env) {
    ...
    if (extract_test_info(test_file_env, &category, &filename)) {
      test_python_example(category, filename);
      free(category);
      free(filename);
    } else {
