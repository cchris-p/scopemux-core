...
def generate_combined_json(
    ...
    result["comment"] = "Generated by ScopeMux Parser"

    # Add AST data if available
    if ast_dict and "ast_root" in ast_dict:
        result["ast_root"] = ast_dict["ast_root"]

    # Add CST data if available
...
def process_directory(dir_path: str, **kwargs) -> Tuple[int, int]:
    ...
    for root, _, files in os.walk(dir_path):
        for file in files:
            ...
            if process_file(file_path, **kwargs):
                success_count += 1
            else:
                failure_count += 1
