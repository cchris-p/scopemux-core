...
ResolutionStatus reference_resolver_resolve_node(ReferenceResolver *resolver, ASTNode *node,
                                                 ...
                                                 /**
                                                  * @brief Language-specific resolution function
                                                  * signature
                                                  */
                                                 typedef ResolutionStatus (*ResolverFunction)(
                                                 ...
                                                 /**
                                                  * @brief Cleanup function for language-specific
                                                  * resolver data
                                                  */
                                                 typedef void (*ResolverCleanupFunction)(
                                                     void *resolver_data);

                                                 // LanguageResolver is already defined in
                                                 // reference_resolver.h No need to redefine it here

                                                 // Language constants for testing
#define LANG_RUST                                 \
                                                   LANG_UNKNOWN // Use LANG_UNKNOWN for tests

                                                 // Use the standard ResolutionStatus from
                                                 // reference_resolver.h No need to redefine it here

                                                 // REMOVE
                                                 // /**
                                                 //  * @brief Symbol information for reference
                                                 //  resolution
                                                 //  */
                                                 // typedef struct {
                                                 //   char *name;           ///< Symbol name
                                                 //   char *qualified_name; ///< Fully qualified
                                                 //   name SymbolType type;      ///< Symbol type
                                                 //   char *file_path;      ///< Source file path
                                                 //   unsigned int line;    ///< Line number
                                                 //   unsigned int column;  ///< Column number
                                                 //   Language language;    ///< Language of the
                                                 //   symbol void *data;           ///< Optional
                                                 //   additional data
                                                 // } Symbol;

                                                 // Symbol functions are implemented in
                                                 // symbol_test_helpers.c Forward declarations only
                                                 Symbol *symbol_new(const char *name,
                                                                    SymbolType type);
                                                 void symbol_free(Symbol *symbol);

                                                 // Function to add a symbol to a symbol table (for
                                                 // tests)
#include <string.h>

                                                 // Simple symbol entry for chaining
                                                 static inline unsigned long
                                                 hash_qualified_name(const char *qualified_name,
                                                                     size_t num_buckets) {
                                                 ...
                                                 static inline bool
                                                 symbol_table_add(GlobalSymbolTable *table,
                                                                  SymbolEntry *entry) {
                                                   if (!table || !entry || !entry->qualified_name)
                                                     return false;

                                                   unsigned long index = hash_qualified_name(
                                                       entry->qualified_name, table->num_buckets);
                                                   SymbolEntry *existing = table->buckets[index];

                                                   // Check for duplicates (same qualified_name +
                                                   // language)
                                                   while (existing) {
                                                     if (strcmp(existing->qualified_name,
                                                 ...
                                                 static inline ResolutionStatus
                                                 resolve_reference(ReferenceResolver *resolver,
                                                                   ASTNode *node,
                                                                   ReferenceType ref_type) {
                                                   // This is a mock implementation that calls the
                                                   // actual API
                                                   if (!resolver || !node)
                                                   ...
                                                   ResolutionStatus status =
                                                       reference_resolver_resolve_node(
                                                           resolver, node, ref_type, "test_symbol",
                                                           LANG_C);
                                                 ...
                                                 // We need to store node extensions in a separate
                                                 // map since ASTNode doesn't have a data field
#include <stdlib.h>

                                                 // Simple linked list of node references for
                                                 // testing
                                                 ...
                                                 // Functions to attach and get extension data for
                                                 // tests
                                                 static inline bool
                                                 ast_node_set_reference(ASTNode *node,
                                                                        ReferenceType type,
                                                                        Symbol *reference) {
                                                   if (!node)
                                                     return false;

                                                   // First check if this node is already in our
                                                   // list
                                                   ...
                                                   NodeRefEntry *entry =
                                                       (NodeRefEntry *)malloc(sizeof(NodeRefEntry));
                                                   if (!entry) {
                                                     log_debug(
                                                         "NodeRefEntry not found for AST Node.");
                                                     return false;
                                                   }
                                                 ...
                                                 ast_node_get_reference(ASTNode *node) {
                                                   if (!node)
                                                     return NULL;
...
#define REF_TYPE_FUNCTION REF_CALL
                                                 ...
                                                 /* Extended private struct for test resolver
                                                  * implementation */
