...
static ASTNode *create_ast_root_node(ParserContext *ctx) {
    ...
    // Create a root node
    ASTNode *root = ast_node_new(NODE_ROOT);
    if (!root) {
    ...
    if (ctx->filename) {
        ...
        ast_node_set_attribute(root, "filename", ctx->filename);
        
        // Extract file basename for display name
        const char *basename = strrchr(ctx->filename, '/');
        if (basename) {
            basename++; // Skip the '/'
            ast_node_set_attribute(root, "basename", basename);
        } else {
            ast_node_set_attribute(root, "basename", ctx->filename);
        }
    }
    ...
    for (size_t i = 0; i < ast_root->num_children; i++) {
        ...
        if (child->name) {
            ...
            if (qualified_name) {
                ...
                ast_node_set_attribute(child, "original_name", child->name);
...
static ASTNode *validate_and_finalize_ast(ASTNode *ast_root, ParserContext *ctx,
    ...
    if (ctx->log_level <= LOG_DEBUG) {
        log_debug("AST finalization: initial=%zu, final=%zu children", 
                 initial_child_count, ast_root->num_children);
    }
...
ASTNode *ts_tree_to_ast_impl(TSNode root_node, ParserContext *ctx) {
    ...
    if (ctx->log_level <= LOG_DEBUG) {
        log_debug("Starting AST generation for %s", 
                 ctx->filename ? ctx->filename : "unknown file");
    }
    ...
    if (ctx->log_level <= LOG_DEBUG) {
        log_debug("AST generation complete with %zu nodes", 
                 final_root ? final_root->num_children : 0);
    }
