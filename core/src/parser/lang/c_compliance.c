 ...
 * !!! 1. Root nodes (translation_unit) -> NODE_ROOT with empty name      !!!!!!
...
static int c_schema_compliance(ASTNode *node, ParserContext *ctx) {
  ...
  // Rule 1: Root nodes (translation_unit) become NODE_ROOT with empty name
  if (strcmp(node->name, "translation_unit") == 0) {
    log_debug("Applying Rule 1: Root node compliance");
    set_node_attributes(node, NODE_ROOT, "", "", "");
    log_debug("Set root node (translation_unit) with empty name/qualified_name");
    return 1;
  }

  // Rule 2a: Comment nodes become NODE_COMMENT
  if (strcmp(node->name, "comment") == 0) {
    // Skip docstring handling as it's covered in Rule 2b
    if (!(node->raw_content && strstr(node->raw_content, "/**") == node->raw_content)) {
      log_debug("Applying Rule 2a: Comment node compliance");
      // Preserve raw content but set other fields consistently
      set_node_attributes(node, NODE_COMMENT, "", "", NULL);
      log_debug("Set comment node");
      return 1;
    }
  }

  // Rule 2b: Docstring nodes become NODE_DOCSTRING with empty name
  // In C, we consider certain comments as docstrings based on content
  if (node->raw_content && strstr(node->raw_content, "/**") == node->raw_content) {
    log_debug("Applying Rule 2b: Docstring node compliance");
    set_node_attributes(node, NODE_DOCSTRING, "", "", NULL);
    log_debug("Set docstring node with empty name/qualified_name");
    return 1;
  }

  // Rule 3: Function nodes become NODE_FUNCTION with proper name
  if (strcmp(node->name, "function_definition") == 0) {
    log_debug("Applying Rule 3: Function node compliance");
  ...
  // Rule 4: Preprocessor includes become NODE_INCLUDE
  if (strcmp(node->name, "preproc_include") == 0) {
    log_debug("Applying Rule 4: Include node compliance");
