...
ResolutionStatus typescript_resolver_impl(ASTNode *node, ReferenceType ref_type, const char *name,
  ...
  return reference_resolver_typescript(node, ref_type, name, symbol_table, resolver_data);
}
...
ResolutionStatus reference_resolver_typescript(ASTNode *node, ReferenceType ref_type,
  ...
  ResolutionStatus result = reference_resolver_javascript(node, ref_type, name, symbol_table, NULL);
...
void typescript_resolver_get_stats(size_t *total, size_t *resolved, size_t *import_resolved,
                                   size_t *property_resolved, size_t *type_resolved,
                                   size_t *interface_resolved, size_t *generic_resolved) {
  if (total)
    *total = ts_resolver_stats.total_lookups;
  if (resolved)
    *resolved = ts_resolver_stats.resolved_count;
  if (import_resolved)
    *import_resolved = ts_resolver_stats.import_resolved;
  if (property_resolved)
    *property_resolved = ts_resolver_stats.property_resolved;
  if (type_resolved)
    *type_resolved = ts_resolver_stats.type_resolved;
  if (interface_resolved)
    *interface_resolved = ts_resolver_stats.interface_resolved;
  if (generic_resolved)
    *generic_resolved = ts_resolver_stats.generic_resolved;
}

/**
 * Reset TypeScript resolver statistics
 */
void typescript_resolver_reset_stats() { memset(&ts_resolver_stats, 0, sizeof(ts_resolver_stats)); }
