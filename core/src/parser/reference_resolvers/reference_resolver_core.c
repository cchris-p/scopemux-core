 ...
 */
bool reference_resolver_register(LanguageType lang, ResolverFunction resolver_func, void *resolver_data) {
 ...
 */
ResolutionResult reference_resolver_resolve(ASTNode *node, ReferenceType ref_type,
                                         const char *name, GlobalSymbolTable *symbol_table) {
    if (!node || !name || !symbol_table) {
        return RESOLUTION_FAILED;
    }
    ...
    if (lang >= 0 && lang < LANG_MAX && 
        resolver_registry[lang].is_registered &&
 ...
 */
void reference_resolver_get_stats(size_t *total_lookups, size_t *resolved_count, size_t *failed_count) {
    if (total_lookups) *total_lookups = resolver_stats.total_lookups;
    if (resolved_count) *resolved_count = resolver_stats.resolved_count;
    if (failed_count) *failed_count = resolver_stats.failed_count;
}
