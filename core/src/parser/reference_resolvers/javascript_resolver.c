 ...
 */
void javascript_resolver_get_stats(size_t *total, size_t *resolved, 
                               size_t *import_resolved, size_t *property_resolved,
                               size_t *prototype_resolved) {
    if (total) *total = js_resolver_stats.total_lookups;
    if (resolved) *resolved = js_resolver_stats.resolved_count;
    if (import_resolved) *import_resolved = js_resolver_stats.import_resolved;
    if (property_resolved) *property_resolved = js_resolver_stats.property_resolved;
    if (prototype_resolved) *prototype_resolved = js_resolver_stats.prototype_resolved;
}

/**
 * Reset JavaScript resolver statistics
 */
void javascript_resolver_reset_stats() {
    memset(&js_resolver_stats, 0, sizeof(js_resolver_stats));
}
