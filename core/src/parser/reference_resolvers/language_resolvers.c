 ...
 */
ResolutionStatus reference_resolver_c_impl(ASTNode *node, ReferenceType ref_type, const char *name,
  ...
  if (entry) {
    ...
    return RESOLUTION_ERROR; // Failed to add reference
  }
  ...
  ResolutionStatus result =
      reference_resolver_generic_resolve_impl(node, ref_type, name, symbol_table);
