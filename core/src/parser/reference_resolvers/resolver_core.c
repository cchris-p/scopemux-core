...
ReferenceResolver *reference_resolver_create_impl(GlobalSymbolTable *symbol_table) {
    ...
    resolver->language_resolvers = (LanguageResolver *)calloc(
        MAX_LANGUAGE_RESOLVERS, sizeof(LanguageResolver));
...
void reference_resolver_free_impl(ReferenceResolver *resolver) {
    ...
    for (size_t i = 0; i < resolver->num_resolvers; i++) {
        if (resolver->language_resolvers[i].cleanup_func) {
            resolver->language_resolvers[i].cleanup_func(
                resolver->language_resolvers[i].resolver_data);
        }
    }
...
void reference_resolver_get_stats_impl(const ReferenceResolver *resolver,
    ...
    if (!resolver) {
        ...
        if (out_resolved_references) *out_resolved_references = 0;
        if (out_unresolved_references) *out_unresolved_references = 0;
        return;
    }
    
    if (out_total_references) *out_total_references = resolver->total_references;
    if (out_resolved_references) *out_resolved_references = resolver->resolved_references;
    if (out_unresolved_references) *out_unresolved_references = 
        resolver->total_references - resolver->resolved_references;
}
    ...
    for (size_t i = 0; i < resolver->num_resolvers; i++) {
        if (resolver->language_resolvers[i].language == language) {
            ...
            if (i < resolver->num_resolvers - 1) {
                memmove(&resolver->language_resolvers[i], 
                        &resolver->language_resolvers[i+1],
