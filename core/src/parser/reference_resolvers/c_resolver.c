...
// Forward declaration of reference_resolver_c function
...
ResolutionStatus c_resolver_impl(ASTNode *node, ReferenceType ref_type, const char *name,
  ...
  return reference_resolver_c(node, ref_type, name, symbol_table, resolver_data);
}
 ...
 */
void c_resolver_get_stats(size_t *total, size_t *resolved, size_t *macro_resolved,
                          size_t *header_resolved, size_t *struct_fields_resolved) {
  if (total)
    *total = c_resolver_stats.total_lookups;
  if (resolved)
    *resolved = c_resolver_stats.resolved_count;
  if (macro_resolved)
    *macro_resolved = c_resolver_stats.macro_resolved;
  if (header_resolved)
    *header_resolved = c_resolver_stats.header_resolved;
  if (struct_fields_resolved)
    *struct_fields_resolved = c_resolver_stats.struct_fields_resolved;
}

/**
 * Reset C resolver statistics
 */
void c_resolver_reset_stats() { memset(&c_resolver_stats, 0, sizeof(c_resolver_stats)); }
