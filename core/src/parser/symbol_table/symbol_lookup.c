...
SymbolEntry *symbol_table_scope_lookup(const GlobalSymbolTable *table, const char *name,
    ...
    result = symbol_table_lookup(table, name);
    if (result) {
        return result;
    }
    ...
    if (current_scope && current_scope[0] != '\0') {
        ...
        result = symbol_table_lookup(table, qualified_name);
        if (result) {
            return result;
        }
    }
 ...
 * @return Number of matching entries found (may exceed max_entries)
 */
size_t symbol_table_get_by_scope(const GlobalSymbolTable *table, const char *scope_prefix,
                               SymbolEntry **out_entries, size_t max_entries) {
    ...
    for (size_t i = 0; i < table->num_buckets; i++) {
        ...
        while (entry) {
            ...
            if (entry->qualified_name && 
                strncmp(entry->qualified_name, scope_prefix, prefix_len) == 0) {
                // Make sure it's an exact scope match
                if (entry->qualified_name[prefix_len] == '.' || 
                    entry->qualified_name[prefix_len] == ':') {
                    if (out_entries && count < max_entries) {
                        out_entries[count] = entry;
                    }
                    count++;
