...
static const char *determine_capture_name(const char *node_type, const char *query_type) {
    ...
    if (strstr(node_type, "name") || 
        strstr(node_type, "identifier") || 
        strcmp(node_type, "name") == 0) {
        return "name";
    }
    
    if (strstr(node_type, "body") || 
        strcmp(node_type, "body") == 0) {
        return "body";
    }
    
    if (strstr(node_type, "parameter") || 
        strstr(node_type, "param") || 
        strcmp(node_type, "parameters") == 0) {
        return "parameters";
    }
    
    if (strstr(node_type, "comment") || 
        strstr(node_type, "docstring") || 
    ...
    // Main node types
    if (strcmp(query_type, "functions") == 0) {
        if (strstr(node_type, "function") || 
            strstr(node_type, "method") || 
            strcmp(node_type, "function_definition") == 0) {
            return "function";
        }
    } else if (strcmp(query_type, "classes") == 0) {
        if (strstr(node_type, "class") || 
            strstr(node_type, "struct") || 
 ...
 */
void process_query(const char *query_type, TSNode root_node, ParserContext *ctx,
    ...
    const TSQuery *query = query_manager_get_query(ctx->query_manager, 
                                                  ctx->language_type, 
                                                  query_type);
    if (!query) {
        if (ctx->log_level <= LOG_DEBUG) {
            log_debug("No query found for type '%s' and language %d", 
                     query_type, ctx->language_type);
        }
        return;
    }
    ...
    while (ts_query_cursor_next_match(cursor, &match)) {
        ...
        for (uint32_t i = 0; i < match.capture_count; i++) {
            ...
            ParseStatus status = create_node_from_match(node_type, NULL, 
                                                      captured_node, &ast_node, ctx);
            
            if (status == PARSE_SUCCESS && ast_node) {
                // Set node metadata
