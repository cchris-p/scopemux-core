...
TreeSitterParser *ts_parser_init(SupportedLanguage language) {
...
void *scopemux_ts_parser_parse_string(TreeSitterParser *parser, const char *source_code, size_t source_code_length) {
...
bool ts_tree_to_ir(TreeSitterParser *parser, void *tree, ParserContext *parser_ctx) {
  ...
  if (ts_node_is_null(root_node)) {
    if (parser->last_error) free(parser->last_error);
  ...
  if (!parser_ctx->root_node) {
    ...
    parser_ctx->root_node = ir_node_create(NODE_MODULE, 
                                         parser_ctx->filename ? parser_ctx->filename : "<unknown>", 
                                         parser_ctx->filename ? parser_ctx->filename : "<unknown>", 
                                         file_range);
    if (!parser_ctx->root_node) {
      if (parser->last_error) free(parser->last_error);
...
size_t ts_extract_comments(TreeSitterParser *parser, void *tree, ParserContext *parser_ctx) {
  ...
  do {
    ...
    if (ts_is_comment(parser, &node)) {
      ...
      // Create source range
      SourceRange range = {
        {start_point.row, start_point.column, start_byte},
        {end_point.row, end_point.column, end_byte}
      };
      
      // Extract the comment text
      char *comment_text = ts_get_node_text(&node, parser_ctx->source_code, parser_ctx->source_code_length);
      if (comment_text) {
...
size_t ts_extract_functions(TreeSitterParser *parser, void *tree, ParserContext *parser_ctx) {
  ...
  do {
    ...
    if (ts_is_function(parser, &node)) {
      ...
      char *signature = ts_extract_function_signature(parser, &node, 
                                               parser_ctx->source_code, 
      ...
      if (!ts_node_is_null(identifier)) {
        name = ts_get_node_text(&identifier, parser_ctx->source_code, parser_ctx->source_code_length);
      }
      ...
      if (function_node) {
        ...
        if (!ts_node_is_null(body)) {
          function_node->raw_content = ts_get_node_text(&body, 
                                                 parser_ctx->source_code, 
                                                 parser_ctx->source_code_length);
        }
        ...
        if (ir_node_add_child(parser_ctx->root_node, function_node)) {
          ...
          // Failed to add as child, free memory
          if (function_node->signature) free(function_node->signature);
          if (function_node->raw_content) free(function_node->raw_content);
...
size_t ts_extract_classes(TreeSitterParser *parser, void *tree, ParserContext *parser_ctx) {
  ...
  do {
    ...
    if (ts_is_class(parser, &node)) {
      ...
      char *name = ts_extract_class_name(parser, &node, 
                                    parser_ctx->source_code, 
                                    parser_ctx->source_code_length);
      ...
      if (class_node) {
        ...
        if (!ts_node_is_null(body)) {
          class_node->raw_content = ts_get_node_text(&body, 
                                               parser_ctx->source_code, 
                                               parser_ctx->source_code_length);
        }
        ...
        if (ir_node_add_child(parser_ctx->root_node, class_node)) {
          ...
          // Failed to add as child, free memory
          if (class_node->raw_content) free(class_node->raw_content);
  ...
  switch (parser->language) {
    case LANG_C:
      ...
      return (strcmp(node_type, "function_definition") == 0 || 
              strcmp(node_type, "declaration") == 0);
  ...
  switch (parser->language) {
    case LANG_C:
      ...
      return (strcmp(node_type, "struct_specifier") == 0 || 
              strcmp(node_type, "enum_specifier") == 0 || 
              strcmp(node_type, "union_specifier") == 0 || 
...
char *ts_extract_function_signature(TreeSitterParser *parser, void *tree_node, const char *source_code, size_t source_code_length) {
  ...
  switch (parser->language) {
    ...
    case LANG_PYTHON: {
      ...
      if (!name_text || !parameters_text) {
        if (name_text) free(name_text);
        if (parameters_text) free(parameters_text);
        return NULL;
      }
...
char *ts_extract_class_name(TreeSitterParser *parser, void *tree_node, const char *source_code, size_t source_code_length) {
  if (!parser || !tree_node || !source_code || source_code_length == 0) {
    return NULL;
  }
  ...
  switch (parser->language) {
    ...
    case LANG_CPP: {
      ...
      if (strcmp(node_type, "struct_specifier") == 0 ||
          strcmp(node_type, "class_specifier") == 0 ||
          strcmp(node_type, "enum_specifier") == 0 ||
          strcmp(node_type, "union_specifier") == 0) {
