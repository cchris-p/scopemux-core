...
static void associate_docstrings_recursive(ASTNode *node, DocstringInfo *docstrings, size_t count) {
  ...
  // Only associate with specific node types that should have docstrings (functions, classes,
  // methods) Variables, control flow, and other nodes should not get automatic docstring assignment
  if (node->type == NODE_FUNCTION || node->type == NODE_CLASS || node->type == NODE_METHOD ||
      node->type == NODE_STRUCT || node->type == NODE_ENUM || node->type == NODE_INTERFACE) {
