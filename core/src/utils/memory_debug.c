...
#define MAX_TRACKED_ALLOCATIONS 1000000 // Increased from 10000 to 1M
#define MAX_TAG_LENGTH 32

// Configuration flags - enabled by default
static bool tracking_enabled = true;
static bool bounds_check_enabled = true;
static bool leak_detection_enabled = true;
...
void memory_debug_configure(bool enable_tracking, bool enable_bounds_check,
...
bool memory_debug_is_valid_ptr(const void *ptr) {
  ...
  for (size_t i = 0; i < allocation_count; i++) {
    ...
    if (bounds_check_enabled && !allocations[i].freed &&
...
void *memory_debug_realloc(void *ptr, size_t size, const char *file, int line, const char *tag) {
  ...
  // Check if this is an untracked pointer
