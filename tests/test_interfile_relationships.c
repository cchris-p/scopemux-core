...
static const char *TEST_MODULE1_CONTENT = "// Test module 1\n"
                                          "\n"
                                          "int global_var = 42;\n"
                                          "\n"
...
static const char *TEST_MODULE2_CONTENT = "// Test module 2\n"
                                          "\n"
...
static bool test_symbol_table_registration() {
  ...
  node.name = STRDUP("test_function", "test_node_name");
  node.qualified_name = STRDUP("module.test_function", "test_node_qualified_name");

  // Register the symbol
  SymbolEntry *entry = symbol_table_register(table, node.qualified_name, &node, "/path/to/file.c",
                                             SCOPE_GLOBAL, LANG_C);
...
static bool test_reference_resolution_simple() {
  ...
  node_def.name = STRDUP("test_function", "test_node_def_name");
  node_def.qualified_name = STRDUP("module.test_function", "test_node_def_qualified_name");
  node_def.language_type = LANG_C;

  ASTNode node_ref;
  memset(&node_ref, 0, sizeof(ASTNode));
  node_ref.type = NODE_FUNCTION_CALL;
  node_ref.name = STRDUP("test_function", "test_node_ref_name");
  ...
  ResolutionStatus result =
      reference_resolver_resolve_node(resolver, &node_ref, REFERENCE_CALL, "module.test_function");
